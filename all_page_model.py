...
def parse_car_details(base_url):
    ...
    while True:
        ...
        car_cards = soup.find_all(class_='css-16kqa8y efwtv890')  # Локатор карточки авто
        ...
        for card in car_cards:
            ...
            price_span = price_element.find('span', {'data-ftid': 'bull_price'}) if price_element else None
            price = price_span.text.strip().replace('\xa0', '') if price_span else 'Не найден'
            city_element = card.find_next(class_='css-1488ad e162wx9x0')
            city = city_element.text.strip() if city_element else 'Не найден'
            image_element = card.find_next(class_='emt6rd1', attrs={'data-ftid': 'bull_image'})
            image_url = image_element.find('img')['src'] if image_element and image_element.find('img') else 'Не найден'
