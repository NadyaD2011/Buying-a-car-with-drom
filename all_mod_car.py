...
def parse_car_brands():
    ...
    for number in range(5):
        car_name = (
            car_elements[number]
            .find("span", class_="css-1kb7l9z e162wx9x0")
            .text.strip()
        )
        ...
        car_brands[car_name] = {"link": car_link, "icon_url": icon_url}
...
def parse_car_details(base_url):
    ...
    params = {"ph": "1", "unsold": "1", "location": "spb"}
    cars = {}
    for page_number in range(1, 11):
        url = (
            f"{base_url}all/page{page_number}/"
            if page_number > 1
            else f"{base_url}all/"
        )
        ...
        for card in car_cards:
            ...
            image_url = (
                image_element.find("img")["src"] if image_element else "Не найден"
            )
            ...
            description_items = description_element.find_all(
                "span", {"data-ftid": "bull_description-item"}
            )
            fuel = (
                description_items[1].text.strip() if len(description_items) > 1 else ""
            )
            transmission = (
                description_items[2].text.strip() if len(description_items) > 2 else ""
            )
            drive = (
                description_items[3].text.strip() if len(description_items) > 3 else ""
            )
            mileage = (
                description_items[4].text.strip().replace("\xa0", "")
                if len(description_items) > 4
                else ""
            )

            price_element = soup.find("span", {"data-ftid": "bull_price"})
            price = (
                price_element.text.replace("\xa0", "") if price_element else "Не найден"
            )
            ...
            link_element = card.find(
                "a",
                class_="g6gv8w4 g6gv8w8 _1ioeqy90",
                attrs={"data-ftid": "bull_title"},
            )
